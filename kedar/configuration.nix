...
in {
  ...
  systemd.services.openvscode-server-shadaj = {
    ...
    serviceConfig = {
      ...
      ExecStart = "${pkgs.bash}/bin/bash -c \"source /etc/profile; eval \\\"\$(${pkgs.openssh}/bin/ssh-agent -s)\\\"; rm -f /run/openvscode-server/shadaj.sock; ${unstable.vscode}/bin/code serve-web --socket-path /run/openvscode-server/shadaj.sock --without-connection-token --extensions-dir /home/shadaj/.vscode/extensions\"";
